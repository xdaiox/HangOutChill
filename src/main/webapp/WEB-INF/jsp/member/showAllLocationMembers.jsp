    ...
    <div class="container-fluid page-body-wrapper">
        ...
        <div class="container">
            <table class="table table-hover">
                ...
                <tbody id="table">
                <jstl:forEach var="normalMember" items="${page.content}">
                    <tr>
                        <th scope="row" class="align-middle">${page.content.indexOf(normalMember)+1}</th>
                        <td class="align-middle">${normalMember.role.roleName}</td>
                        <td class="align-middle">${normalMember.account}</td>
                        <td class="align-middle">${normalMember.reallName}</td>
                        <td class="align-middle">${normalMember.birthdate}</td>
                        <td class="align-middle">${normalMember.gender}</td>
                        <td class="align-middle">
                            <div style="display: flex">
                                <form method="post" action="${contextRoot}/back/authority">
                                    <input type="hidden" name="_method" value="PUT">
                                    <input type="hidden" name="id" value="${normalMember.id}">
                                    <jstl:choose>
                                        <jstl:when test="${normalMember.enabled == false}">
                        ...
                        <td class="align-middle">
                            <div style="display: flex">
                                ...
                                <form>
                                    <input type="hidden" name="id" value="${normalMember.id}" id="memberId">
            ...
            <nav aria-label="Page navigation example" style="text-align: center" id="pageNum" >
                <jstl:forEach var="pageNumber" begin="1" end="${page.totalPages}">
                    <jstl:choose>
                        ...
                        <jstl:otherwise>
                            <a href="${contextRoot}/back/showNormalMember?pageNum=${pageNumber}">${pageNumber}</a>
                        </jstl:otherwise>
                    </jstl:choose>
                </jstl:forEach>
            </nav>
